...
# === GRID CONFIG ===
width, height = 53, 7          # GitHub contribution grid
cell = 10                      # pixels per cell
frames_total = 90              # shorter for small GIF
...
def draw_frame():
    img = Image.new("RGB", (width * cell, height * cell), "white")
    draw = ImageDraw.Draw(img)

    # background grid
    for y in range(height):
        for x in range(width):
            draw.rectangle([(x*cell, y*cell),
                            ((x+1)*cell-1, (y+1)*cell-1)],
                           fill=(240,240,240))

    # draw agents
    for a in agents:
        draw.rectangle([(a.x*cell, a.y*cell),
                        ((a.x+1)*cell-1, (a.y+1)*cell-1)],
                       fill=a.color)
    # draw projectiles
...
def move_agent(agent):
    ...
    # Sentinel barely moves

def agent_fire(agent):
    if agent.cooldown > 0:
        agent.cooldown -= 1
        return
    chance = {"steady":0.07,"rapid":0.15,"teleport":0.08,"precise":0.05,"diagonal":0.1}[agent.behavior]
    if random.random() < chance:
        ...
        if dist != 0:
            ...
            projectiles.append(Projectile(agent.x, agent.y, ux, uy, agent.color))
            agent.cooldown = 5
...
for _ in range(frames_total):
    ...
    # snake random turn occasionally
